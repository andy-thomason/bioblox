    ...
    IEnumerator CreateLevelsCoru()
    {
        foreach (string level_name in dataController.level_index)
        {
            if (level_name != "")
            {
                ...
                LevelMap.transform.GetChild(level_index).GetComponent<LevelStructureV2>().init(level_name, level_position[level_index], level_index);
                level_index++;
            }
        }
        /*
