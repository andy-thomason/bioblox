...
public class CLONEPDB_parser {
	...
	static public List<PDBmolecule> parse(string asset_name) {
		...
		PDBmolecule cur = new PDBmolecule();
		List<PDBmolecule> result = new List<PDBmolecule>();
		...
		using (StringReader reader = new StringReader(pdbTA.text)) {
			...
			while ((line = reader.ReadLine()) != null) {
				...
				if (kind == "ATOM  ") { // && line.Substring(13 - 1, 4) == " N  ")
					...
					string name = PDBmolecule.encode(line[12], line[13], line[14], line[15]) + " " + id + " " + serial;
					/*if (name == PDBmolecule.atom_N) {
						residues.Add(PDBmolecule.encode(line[17], line[18], line[19], ' '));
					...
					if (serial >= 0) {
						while (serial >= serial_to_atom.Count)
							serial_to_atom.Add(-1);
	...
	static public Dictionary<string, PDBmolecule> mols = new Dictionary<string, PDBmolecule>();

	public static PDBmolecule get_molecule(string name) {
		if (!mols.ContainsKey(name)) {
			string[] parts = name.Split('.');
			List<PDBmolecule> list = parse(parts[0]);
