...
public class CUIColorPicker : MonoBehaviour
{
    ...
    bool is_color_panel_activated = false;
    GameManager gm;

    public void SpawnColorPanel()
    {
        ColorPanel.SetActive(!is_color_panel_activated);
        is_color_panel_activated = !is_color_panel_activated;
    }

    public void SpawnColorPanel(bool status)
    {
        ColorPanel.SetActive(status);
        is_color_panel_activated = status;
    }
    ...
    {
        return transform.GetChild(1).transform.Find(name).gameObject;
    }
    ...
    private void Setup(Color inputColor)
    {
        ...
        Action idle = () =>
        {
            if (Input.GetMouseButtonDown(0) && is_color_panel_activated)
            {
                Vector2 mp;
    ...
    void Start()
    {
        ...
        Color = gm.default_background_color;
    }
