...
public class AminoConnectionHolder : MonoBehaviour {
    ...
    {
        sfx.PlayTrack(SFX.sound_index.button_click);
        FindObjectOfType<BioBlox>().molecules[1].GetComponent<PDB_mesh>().SelectAminoAcid(ID_button2,false);
        FindObjectOfType<AminoSliderController>().UpdateCurrentButtonA2(ID_button2);			
		FindObjectOfType<BioBlox>().molecules[0].GetComponent<PDB_mesh>().SelectAminoAcid(ID_button1,false);
